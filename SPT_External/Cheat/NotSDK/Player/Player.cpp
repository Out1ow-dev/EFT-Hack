...
Vector3 Player::GetTransformPosition(uintptr_t transform)
{
	...
	TransformAccessReadOnly pTransformAccessReadOnly = m.read_memory<TransformAccessReadOnly>(transform + 0x38);
	unsigned int Index = m.read_memory<unsigned int>(transform + 0x40);
	TransformData transform_data = m.read_memory<TransformData>(pTransformAccessReadOnly.pTransformData + 0x18);

	if (transform_data.pTransformArray && transform_data.pTransformIndices)
	{
		result = m.read_memory<__m128>(transform_data.pTransformArray + 0x30 * Index);
		int transform_index = m.read_memory<int>(transform_data.pTransformIndices + 0x4 * Index);
		int safe = 0;
		while (transform_index >= 0 && safe++ < 200)
		{
			Matrix34 matrix = m.read_memory<Matrix34>(transform_data.pTransformArray + 0x30 * transform_index);
			...
			transform_index = m.read_memory<int>(transform_data.pTransformIndices + 0x4 * transform_index);
		}
	}
